...
def generate_tss_binned_coords(coordinates_info, upstream_range, downstream_range, bin_size):
	...
	#Index([u'chrom', u'tss_midpoint', u'bin_start', u'bin_end', u'name2', u'strand', u'chrom_start', u'chrom_end'], dtype='object')
	...
	tss_coord_df1.to_csv(join(output_dir, "tss_coordinate_out_test.bed"), sep="\t", index=False, header=False)

	return(tss_coord_df1)



def generate_tss_binned_coords_otherway(coordinates_info, upstream_range, downstream_range, bin_size, coord_outfile):
...
def load_cpg_coord_pybedtool_object(file_name_with_full_path):
...
def main():
	...
	refgene_coord_df = final_refgene_model(refgene_file, refgene_model_out_file)
	tss_coord_df = generate_tss_binned_coords(refgene_coord_df, 1000, 1000, 100 )

	refseq_bed_file = load_tss_coord_pybedtool_object(join(output_dir, "tss_coordinate_out_test.bed"))
